// /frontend/app/api/cron/check-games/route.ts (FINAL PRODUCTION-READY CODE V2)
...
async function calculateFranchiseScorigamiProbability(supabase: SupabaseClient, game: MLBGame, dbAwayId: number, dbHomeId: number): Promise<ScorigamiProbabilityResult | null> {
    ...
    // ✨ THE FIX IS HERE: Changed from <= 0 to < 0 to allow the 9th inning to be calculated.
    ...
    for (let k = 0; k <= MAX_ADDITIONAL_RUNS_TO_CHECK; k++) {
        ...
        if (probOfKMoreRuns < 0.00001 && k > 0) break; // Allow k=0 to always run
